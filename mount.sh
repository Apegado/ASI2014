...
mkdir --parents $puntoMontaje > /dev/null
...
grep -q "$nombreDis" fstab.tmp && echo "El dispositivo ya esta en fstab" || (echo "$montaje" >> fstab.tmp && mv fstab.tmp /etc/fstab && echo "Dispositivo añadido a fstab.")
mount -a > /dev/null